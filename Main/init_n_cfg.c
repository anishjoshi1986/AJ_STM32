...
void init_stm32clksys()
{
	...
	pRCC->CR = (pRCC->CR & ~RCC_CR_MASK) | (STM32_Pack_RCC_CR(&RCC_CR) & RCC_CR_MASK);
	...
	RCC_CFGR.SW = SW_VAL;
	pRCC->CFGR = (pRCC->CFGR & ~RCC_CFGR_MASK) | (STM32_Pack_RCC_CFGR(&RCC_CFGR) & RCC_CFGR_MASK);
	...
	pRCC->AHBENR = (pRCC->AHBENR & ~RCC_AHBENR_MASK) | (STM32_Pack_RCC_AHBENR(&RCC_AHBENR) & RCC_AHBENR_MASK);
	...
	pRCC->APB1ENR = (pRCC->APB1ENR & ~RCC_APB1ENR_MASK) | (STM32_Pack_RCC_APB1ENR(&RCC_APB1ENR) & RCC_APB1ENR_MASK);
	...
	pRCC->APB2ENR = (pRCC->APB2ENR & ~RCC_APB2ENR_MASK) | (STM32_Pack_RCC_APB2ENR(&RCC_APB2ENR) & RCC_APB2ENR_MASK);
...
void init_stm32timersys()
{
	...
	TS_TIM6.BTIMx_Cfg.freq = (uint16_t)(1/PERIOD_1MS);
	BTIMx_Init(&TS_TIM6);

	TS_TIM7.pBTIMx = pTIM7;
	TS_TIM7.BTIMx_Cfg.freq = (uint16_t)(1/PERIOD_10MS);
	BTIMx_Init(&TS_TIM7);
